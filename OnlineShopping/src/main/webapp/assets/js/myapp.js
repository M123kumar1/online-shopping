$(function() {
	...
	//to tacle the csrf token
	var token=$('meta[name="_csrf"]').attr('content');
	var header=$('meta[name="_csrf_header"]').attr('content');
	
	if(token.length>0 && header.length>0){
		//set the token request for the ajax request
		$(document).ajaxSend(function(e,xhr,options){
			xhr.setRequestHeader(header,token);
		});
	}
	
	//code for jquery dataTables from datatables.net
	...
	$categoryForm.validate({
		...
		messages:{
			name:{
				...
				minlength:'The category name should not be less then two character'
			},
			description:{
				required:'Please add descrition for this category'
			}
	...
	$loginForm.validate({
		...
		messages:{
			username:{
				...
				email:'Plese enter the valid email address'
			},
			password:{
	...
	if(currentCount !==originalCount){
		...
		if(currentCount<1 || currentCount>3){
			...
			bootbox.alert({
				size:'medium',
				title:'Error',
				message:'Product count should be minimun 1 and maximum 3!'
			});
		}
		else{
